...
func UploadImageAPI(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == http.MethodPost {
		...
		if conf.Mode != "p" && !valid {
			errJsonMsg("Invalid file type. Only .jpg, .jpeg, .png, and .webp are allowed.", w)
		...
		if conf.BotToken == "" || conf.ChannelName == "" {
			// 模拟失败响应，tgstate 服务未配置
			res = conf.UploadResponse{
				Code:    0,
				Message: "tgstate_not_configured",
				ImgUrl:  "",
			}
			log.Printf("tgstate API called but not configured - BotToken: %s, ChannelName: %s", 
				func() string {
					if conf.BotToken == "" {
						return "empty"
					}
